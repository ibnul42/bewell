...
const Accordion = ({ title, items, slColor }) => {
    ...
    return (
        ...
        <div className="max-w-7xl mx-auto p-2 font-semibold">
            {items && items.map((item, index) => (
                <div key={index} className="flex flex-col gap-3 bg-white rounded-md my-2 md:my-3 shadow-sm text-[#666666]">
                    <div onClick={() => expandHandler(index)} className={`px-6 py-2 flex gap-1 justify-between cursor-pointer`}>
                        <p className='w-[95%] font-bold text-lg md:text-xl'><span className={handleColor()}>{item.serial + '. '}</span>{item.title}</p>
